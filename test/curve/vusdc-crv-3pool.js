...
const Crv3PoolStrategy = artifacts.require('Crv3PoolStrategyUSDC')
const Controller = artifacts.require('Controller')

contract('vUSDC Pool with Crv3PoolStrategy', function (accounts) {
  ...
  beforeEach(async function () {
    await setupVPool(this, {
      ...
      pool: VUSDC,
      strategy: Crv3PoolStrategy,
      feeCollector: accounts[9],
      strategyType: 'crv',
    })
    this.newStrategy = Crv3PoolStrategy
    pool = this.pool
  ...
  shouldBehaveLikePool('vUsdc', 'USDC', '3CRV', accounts)
  shouldBehaveLikeStrategy('vUsdc', 'USDC', '3CRV', accounts)
  ...
  describe('Crv3PoolStrategy: USDC Functionality', function() {
    it('Should calculate fees properly and reflect those in share price', async function () {
      ...
      expect(price2).to.be.bignumber.gt(price1, 'Share value should increase (1)')
      // Time travel to generate earning
      await time.increase(30*24*60*60)
      await deposit(pool, collateralToken, 20, user3)
      await time.increase(30*24*60*60)
      let prevLpRate2 = await strategy.prevLpRate()
      expect(prevLpRate2).to.be.bignumber.gt(prevLpRate, 'LP Rate should increase (1)')
      prevLpRate = prevLpRate2
      await pool.rebalance()
      prevLpRate2 = await strategy.prevLpRate()
      expect(prevLpRate2).to.be.bignumber.gt(prevLpRate, 'LP Rate should increase (2)')
      const price3 = await pool.getPricePerShare()
      expect(price3).to.be.bignumber.gt(price2, 'Share value should increase (2)')
    })
    ...
    xit('Crv3PoolStrategy: USDC APY', async function() {
      ...
      console.log(`avg gas used by rebalance: ${gasUsed.div(new BN(30))}`)
    })
  })
})
