...
describe('Vesper Pool: Admin only function tests', function () {
  ...
  describe('Add strategy test', function () {
    it('Should add strategy and make it active', async function () {
      ...
      const tx = pool.addStrategy(strategy.address, ...Object.values(config))
      await expect(tx)
        .to.emit(pool, 'StrategyAdded')
        .withArgs(strategy.address, config.interestFee, config.debtRatio, config.debtRate)
      expect((await pool.strategy(strategy.address)).active, 'Strategy should be active').to.be.true
    })
    ...
    it('Should revert if strategy is already active', async function () {
      ...
      const tx = pool.addStrategy(strategy.address, ...Object.values(strategyConfig.config))
    ...
    it('Should revert if strategy address is zero', async function () {
      const tx = pool.addStrategy(Address.ZERO, ...Object.values(strategyConfig.config))
  ...
  describe('Migrate strategy', function () {
    it('Should migrate strategy', async function () {
      const config = strategyConfig.config
      await pool.addStrategy(strategy.address, ...Object.values(config))
      ...
      await expect(tx)
        .to.emit(pool, 'StrategyMigrated')
        .withArgs(strategy.address, newStrategy.address, config.interestFee, config.debtRatio, config.debtRate)
      expect((await pool.strategy(newStrategy.address)).active, 'Strategy should be active').to.be.true
      expect((await pool.strategy(strategy.address)).active, 'Old strategy should be de-active').to.be.false
    })

    it('Should migrate strategy and replace in strategies array', async function () {
      await pool.addStrategy(strategy.address, ...Object.values(strategyConfig.config))
      const newStrategy = await createStrategy(strategyConfig, pool.address, {addressListFactory})
      expect(await pool.strategies(0)).to.be.eq(strategy.address, 'strategies[0] should be old strategy')
      await pool.migrateStrategy(strategy.address, newStrategy.address)
      expect(await pool.strategies(0)).to.be.eq(newStrategy.address, 'strategies[0] should be new strategy')
    })
    ...
    it('Should revert if new strategy is active', async function () {
      ...
      await pool.addStrategy(newStrategy.address, ...Object.values(config))
      const tx = pool.migrateStrategy(strategy.address, newStrategy.address)
      // 15 = STRATEGY_IS_ACTIVE
