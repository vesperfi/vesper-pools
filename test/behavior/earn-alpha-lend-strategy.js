...
function shouldBehaveLikeEarnAlphaLendStrategy(strategyIndex) {
  let strategy, user2, pool, collateralToken

  describe('EarnAlphaLendStrategy specific tests', function () {
    ...
    it('Should increase DAI balance on rebalance', async function () {
      ...
      expect(tokenBalanceAfter).to.be.gt(tokenBalanceBefore, 'Should increase vDAI balance in AlphaLend strategy')
      await timeTravel()
...
module.exports = {shouldBehaveLikeEarnAlphaLendStrategy}
