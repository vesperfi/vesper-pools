...
// Copied from AaveLeverageStrategy.sol
...
contract AaveLeverageAvalancheStrategy is Strategy, FlashLoanHelper, AaveCoreAvalanche {
    ...
    uint256 public minBorrowRatio = 3_500; // 35%
    uint256 public maxBorrowRatio = 4_500; // 45%
    ...
    function totalValue() public view virtual override returns (uint256 _totalValue) {
        ...
        // If rewardToken is different than collateralToken then get the collateral equivalent of rewardAccrued
        if (rewardToken != address(collateralToken) && _rewardAccrued > 0) {
            (, _rewardAccrued, ) = swapManager.bestOutputFixedInput(
                rewardToken,
        ...
        (uint256 _supply, uint256 _borrow) = getPosition();
        _totalValue = _rewardAccrued + collateralToken.balanceOf(address(this)) + _supply - _borrow;
    }
