...
const VWBTC = require('../../helper/polygon/poolConfig').VWBTC
const Address = require('../../helper/polygon/address')

const PoolAccountant = 'PoolAccountant'
const strategyName = 'AaveStrategyPolygonWBTC'
...
const deployFunction = async function ({getNamedAccounts, deployments}) {
  ...
  const poolProxy = await deploy(VWBTC.contractName, {
    from: deployer,
    log: true,
    args: VWBTC.poolParams, // Constructor args
    // proxy deployment
    proxy: {
      proxyContract: 'OpenZeppelinTransparentProxy',
      ...
      execute: {
        init: {
          ...
          args: [...VWBTC.poolParams, accountantProxy.address, Address.ADDRESS_LIST_FACTORY],
        },
      },
    },
  })
  ...
  const vWBTCStrategy = await deploy(strategyName, {
    from: deployer,
    log: true,
  ...
  await execute(
    ...
    'addStrategy',
    vWBTCStrategy.address,
    config.interestFee,
    config.debtRatio,
    config.debtRate
  )
  await execute(VWBTC.contractName, {from: deployer, log: true}, 'updateFeeCollector', config.feeCollector)
  await execute(VWBTC.contractName, {from: deployer, log: true}, 'updateWithdrawFee', config.withdrawFee)

  // Prepare id of deployment, next deployment will be triggered if id is changed
  const poolVersion = await read(VWBTC.contractName, {}, 'VERSION')
  const poolAccountantVersion = await read(PoolAccountant, {}, 'VERSION')
  deployFunction.id = `${VWBTC.poolParams[1]}-v${poolVersion}_${poolAccountantVersion}`
  return true
}
module.exports = deployFunction
module.exports.tags = ['VWBTC-P-v3.0.8']
